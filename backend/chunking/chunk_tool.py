...
_REPAIR_STOPWORDS = {
    ...
    # Connecting/explanatory synthesis words an LLM repair commonly uses to link a
    # row's own cell content into a sentence -- real gap found during the 4-Aug
    # merge: a discarded parallel implementation (validate_repaired_chunks) had
    # accumulated this domain-specific set independently, and testing it against
    # THIS function (the one actually in use) showed the same synthesis words
    # ("indicates", "recommended", "context", ...) tripped traceability here too.
    "context", "indicates", "indicating", "indicated", "event", "issue", "issues",
    "observed", "resulting", "triggered", "encountered", "noted", "affected",
    "recommended", "potential", "problem", "factors", "lead", "related", "initial",
    "activation", "overheating", "upon", "enabling", "leading", "alert", "within",
    "system", "determined", "showing", "another", "also", "contributing",
    "specific", "required", "possible", "show", "occurrence", "showed", "results",
    "present", "applicable", "identified", "includes", "checking", "replacing",
    "part", "procedure", "basis", "checked", "situation", "linked", "types",
    "servo", "suggests", "causes", "due", "turned", "condition", "detected",
    "contributor", "indications", "arise", "activated", "requires", "significant",
    "involves", "characterized", "occurrences", "happen", "encountering", "reason",
    "denotes", "regarding", "sporadic", "component", "operation", "relates",
    "notable", "surrounded", "relevant", "marked", "checks", "details", "document",
    "happened", "recorded", "relation", "relating", "scenario", "may",
    "furthermore", "addition", "following", "associated", "states", "section",
    "table", "row",
}
...
def chunk_blocks(
    ...
    def _process_table_task(block_val, active_sec, prec_context, block_ref, heading_lead_val):
        ...
        # Real bug found live during this merge, 3-Aug: this used to call
        # classify_table()/repair_table_with_llm() -- names that don't exist
        # anywhere in this file (only _classify_table_for_repair/
        # _repair_table_with_llm do), and the repair call was missing the
        # required ref_fn argument. Also silently ignored repair_hard_tables
        # (config gate to disable this LLM call entirely) -- restored below.
        if repair_hard_tables and config is not None:
            classification = _classify_table_for_repair(block_val, active_sec, prec_context)
            if classification["needs_llm"]:
                logger.info("Table needs LLM repair: %s (reasons: %s)", block_val.get("block_id"), classification["reasons"])
                llm_chunks = _repair_table_with_llm(block_val, config, active_sec, prec_context, _local_ref)
    ...
    for block in blocks:
        ...
        if btype in ATOMIC_TYPES:
            ...
            if btype == "table":
                # Hard-table LLM repair (repair_hard_tables gate), the alarm/
                # troubleshooting/warning/model-column extractors, and the
                # deterministic fallback are all inside _process_table_task now —
                # submitted to the ThreadPoolExecutor below so slow LLM-repair
                # calls for different tables run concurrently. pacing.pace()
                # inside _repair_table_with_llm still serializes the actual
                # rate-limited LLM calls via its per-key lock (see
                # backend/core/pacing.py) — this only parallelizes the
                # surrounding work (classification, deterministic extraction),
