...
ALL_STEPS = ["categorize", "page_profile", "pdf_extraction", "vision_enrichment",
             "chunk", "enrich_chunks", "embed", "index"]
...
def test_runs_end_to_end():
    cfg = PipelineConfig.from_dict({"route": "diagram_heavy", "steps": ALL_STEPS, "route_gates": GATES})
    out = build_pipeline(_registry(), cfg).invoke({"document_id": "d1"})
    assert out["errors"] == []
    assert out["chunks"] and out["chunks"][0]["indexed"] is True  # reached the last step
...
def test_route_branches_on_gate():
    ...
    diagram = PipelineConfig.from_dict({"route": "diagram_heavy", "steps": ALL_STEPS, "route_gates": GATES})
    text = PipelineConfig.from_dict({"route": "text_default", "steps": ALL_STEPS, "route_gates": GATES})
    assert _has_caption(build_pipeline(_registry(), diagram).invoke({"document_id": "d1"}))
    assert not _has_caption(build_pipeline(_registry(), text).invoke({"document_id": "d1"}))
...
def test_failing_tool_degrades_gracefully():
    ...
    reg = _registry()
    reg.register(Boom())
    cfg = PipelineConfig.from_dict({"route": "text_default", "steps": ["boom", "chunk"]})
    out = build_pipeline(reg, cfg).invoke({"document_id": "d1", "blocks": [{"text": "x"}]})
